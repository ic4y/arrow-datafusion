...
use crate::physical_plan::expressions::count_fly::CountAccumulatorFly;
use crate::physical_plan::{
    AccumulatorFly, AggregateExpr, DisplayFormatType, Distribution, ExecutionPlan,
...
#[derive(Debug)]
pub struct HashAggregateExecFly {
...
#[async_trait]
impl ExecutionPlan for HashAggregateExecFly {
    ...
    fn with_new_children(
        ...
        match children.len() {
            1 => Ok(Arc::new(HashAggregateExecFly::try_new(
                self.mode,
            ...
            _ => Err(DataFusionError::Internal(
                "HashAggregateExecFly wrong number of children".to_string(),
            )),
        }
    }
    ...
    fn fmt_as(
        ...
        match t {
            DisplayFormatType::Default => {
                write!(f, "HashAggregateExecFly: mode={:?}", self.mode)?;
...
async fn compute_grouped_hash_aggregate(
    ...
    accumulators.accumulator_items=create_accumulators(&aggr_expr).unwrap();
    timer.done();
    let dt = Local::now();
    let mut i = 0;
    ...
    println!(
        "group_aggregate_batch usage millis: {}, batch count: {}。",
        Local::now().timestamp_millis() - dt.timestamp_millis(),
        i
    );
...
type AccumulatorItem = Box<dyn AccumulatorFly>;
...
fn aggregate_batch(
    ...
    accumulators
        ...
        .try_for_each(|(accum, expr)| {
            ...
            // 1.3
            match mode {
                AggregateMode::Partial => accum.update_batch(0, values),
                AggregateMode::Final | AggregateMode::FinalPartitioned => {
                    accum.merge_batch(0,values)
                }
            }
        })
}
            ...
            match mode {
                AggregateMode::Partial => {
                    let res = ScalarValue::iter_to_array(
                        //Todo 这里可以优化为指获取一次state_all
                        accumulators.accumulator_items[x].state_all().unwrap()[y].clone()
                    )
                        .map_err(DataFusionError::into_arrow_external_error)?;
                ...
                AggregateMode::Final | AggregateMode::FinalPartitioned => {
                    let res = accumulators.accumulator_items[x].evaluate_all().map_err(DataFusionError::into_arrow_external_error)?;
                    columns.push(res);
                }
            }
        }
...
fn create_accumulators(
    ...
    aggr_expr
        .iter()
        .map(|expr| {
            create_fly_accumulator()
        })
        .collect::<Result<Vec<_>>>()
}

fn create_fly_accumulator() -> Result<Box<dyn AccumulatorFly>> {
...
fn finalize_aggregation(
    ...
    match mode {
        AggregateMode::Partial => {
            ...
            let a = accumulators
                .iter()
                .map(|accumulator| accumulator.state(0))
                .map(|value| {
                    value.map(|e| {
        ...
        AggregateMode::Final | AggregateMode::FinalPartitioned => {
            ...
            accumulators
                .iter()
                .map(|accumulator| accumulator.evaluate(0).map(|v| v.to_array()))
...
mod tests {
    ...
    async fn check_aggregates(input: Arc<dyn ExecutionPlan>) -> Result<()> {
        ...
        let partial_aggregate = Arc::new(HashAggregateExecFly::try_new(
            AggregateMode::Partial,
            groups.clone(),
            aggregates.clone(),
            input,
        ...
        let merged_aggregate = Arc::new(HashAggregateExecFly::try_new(
            AggregateMode::Final,
            final_group
                .iter()
                .enumerate()
    ...
    async fn test_drop_cancel_without_groups() -> Result<()> {
        ...
        let hash_aggregate_exec = Arc::new(HashAggregateExecFly::try_new(
            AggregateMode::Partial,
            groups.clone(),
            aggregates.clone(),
            blocking_exec,
